...
public class PetDao {
    ...
    public void buscarNome(String nome) throws  IOException{
        List<Pet> pets = listarPets();
        List<Pet> petsComONome = new ArrayList<>();

        for (Pet pet : pets) {
            if (pet.getNomeSobrenome().contains(nome)) {
                petsComONome.add(pet);
            }
        }
        if(petsComONome.isEmpty()){
            return;
        }
        for (Pet string : petsComONome) {
